...
export function exportCharacterToImprovedInitiative(
  character: Character,
): string {
  ...
  output.Description += s.characterHook
    ? s.characterHook?.map((part) => part.string).join('') + '.'
    : '';
  output.Description += s.personality
    ? ' Personality Trait: ' + s.personality.name + '.'
    : '';
  ...
  output.Speed =
    s.speeds?.map((speed) => speed.string.replace('ft', 'ft.')) || [];
  ...
  output.DamageVulnerabilities =
    s.vulnerabilities?.map((vulnerability) => vulnerability.string) || [];
  output.DamageResistances =
    s.resistances?.map((resistance) => resistance.string) || [];
  output.DamageImmunities =
    s.immunities?.map((immunity) => immunity.string) || [];
  output.ConditionImmunities =
    s.conditionImmunities?.map(
      (conditionImmunity) => conditionImmunity.string,
    ) || [];

  output.Senses =
    s.senses?.map((sense) => sense.string.replace('ft', 'ft.')) || [];
  output.Languages =
    s.languages?.map((language) => language.string.replace('ft', 'ft.')) || [];
  ...
  output.BonusActions =
    s.bonusActions?.map((bonusAction) => {
      return {
  ...
  output.LegendaryActions =
    s.legendaryActions?.map((legendaryAction) => {
      return {
