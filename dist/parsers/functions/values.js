...
function calculateValue(value, character, variant = undefined, isEnchantment = false) {
    ...
    if ('dice' in value) {
        ...
        const unitEnd = currentUnitValue > availableUntil ? availableUntil : currentUnitValue;
        const unitStart = dice.availableAt || 1;
        const unitInterval = dice.unitInterval || 1;
        const diceIncrement = dice.diceIncrement || 0;
        const additionalDice = Math.floor((unitEnd - unitStart) / unitInterval) * diceIncrement || 0;
        let totalDice = dice.dice + additionalDice;
        if (totalDice < 1) {
            totalDice = 1;
        }
    ...
    if ('expression' in value && value.expression) {
        ...
        const expressionResult = (0, expressions_1.parseExpressionNumeric)(expression, character);
    ...
    if ('dice' in value) {
        ...
        part.string = (0, numbers_1.numberToWord)(average);
...
function addAdditionalDescriptionParts(part, type = '', average = 0) {
    ...
    switch (type) {
        ...
        case '+':
            ...
            break;
        case '-st-nd-rd':
            part.string += (0, numbers_1.addOrdinal)(average);
            part.number = average;
            part.type = 'ordinal';
            break;
        case 'feet':
